...
export default function GenerateQuestionsPage() {
  ...
  const fetchMaterialTitle = async () => {
    try {
      ...
      setMaterialTitle(data.data?.title || "Study Material");
    } catch (error) {
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      // First, get the quiz ID from the user or create a new quiz
      // For now, we'll need a quiz ID to attach questions to
      const quizId = sessionStorage.getItem("currentQuizId");

      if (!quizId) {
        alert("Please create a quiz first before generating questions");
      ...
      const res = await fetch(
        ...
        }/ai/questions/generate`,
        {
          ...
          body: JSON.stringify({
            ...
            difficulty: difficulty.toUpperCase(), // Backend expects EASY, MEDIUM, HARD
            quizId,
          }),
        }
      );
      ...
      if (res.ok) {
        ...
        setSelectedQuestions(
          Array.from({ length: formattedQuestions.length }, (_, i) => i)
        );
        alert(
          `Successfully generated ${formattedQuestions.length} questions! 🎉`
        );
      } else {
        ...
        {!isGenerating && generatedQuestions.length === 0 && (
          <Card className="p-12 text-center border-2 border-dashed border-border">
            ...
            <p className="text-muted-foreground">
              Configure your settings above and click "Generate" to create AI
