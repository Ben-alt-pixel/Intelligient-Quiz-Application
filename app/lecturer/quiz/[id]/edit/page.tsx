...
export default function EditQuizPage() {
  ...
  const fetchQuiz = async () => {
    try {
      const res = await fetch(`/api/lecturer/quizzes/${params.id}`)
      if (res.ok) {
        const data = await res.json()
        setTitle(data.quiz.title)
        setDescription(data.quiz.description || "")
        setTimeLimit(data.quiz.timeLimit.toString())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await quizService.updateQuiz(params.id, {
          title,
          description,
    ...
    <main className="min-h-screen bg-gradient-to-br from-primary/5 to-secondary/5">
      ...
      <header className="bg-primary text-primary-foreground shadow-lg">
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-6">
          ...
          <p className="text-primary-foreground/80 text-sm mt-1">Update your quiz questions and settings</p>
        </div>
      </header>
          ...
          <Card className="p-8 border-2 border-border">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <Input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="border-border" />
              </div>
            ...
            <div className="space-y-6">
              {questions.map((question, questionIndex) => (
                <Card key={question.id} className="p-6 border-2 border-border">
                  <h3 className="text-lg font-semibold text-foreground mb-4">Question {questionIndex + 1}</h3>
